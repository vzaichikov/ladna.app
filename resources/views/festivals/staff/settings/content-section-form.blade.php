...
<x-festivals.staff.workspace :$account :$edition :permissions="$workspacePermissions">
    ...
    <x-ui.panel>
        <form method="POST" action="{{ $section->exists ? route('dashboard.accounts.festivals.content.update', [$account, $edition, $section]) : route('dashboard.accounts.festivals.content.store', [$account, $edition]) }}" class="grid gap-5 sm:grid-cols-2">
            ...
            <label class="sm:col-span-2"><span class="crm-label">{{ __('app.content') }}</span><textarea name="body_html" rows="12" class="crm-field" data-studio-rules-editor data-editor-height="360">{{ old('body_html', $section->body_html) }}</textarea>@error('body_html') <span class="crm-help">{{ $message }}</span> @enderror</label>
