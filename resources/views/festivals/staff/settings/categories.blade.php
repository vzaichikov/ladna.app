...
<x-festivals.staff.workspace :$account :$edition :permissions="$workspacePermissions">
    ...
    <div class="space-y-4">
        @foreach($edition->categories as $category)
            @php($categoryEditId = 'festival-category-edit-'.$category->id)
            <article class="rounded-2xl border border-stone-200 bg-white p-5 shadow-crm">
                <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div><h2 class="text-lg font-semibold">{{ $category->name }}</h2><p class="mt-1 text-sm text-slate-500">{{ $category->options->pluck('label')->join(' · ') ?: __('app.festival_no_classification_selected') }} · {{ trans_choice('app.festival_entry_usage_count', $category->entries_count, ['count' => $category->entries_count]) }}</p></div>
                    <x-festivals.settings-actions :active="$category->is_active" :toggle-route="route('dashboard.accounts.festivals.categories.toggle', [$account, $edition, $category])" :move-route="route('dashboard.accounts.festivals.categories.move', [$account, $edition, $category])" :edit-target="$categoryEditId" />
                </div>
                <div id="{{ $categoryEditId }}" class="mt-4 hidden gap-3 rounded-xl bg-stone-50 p-4">
                    <x-festivals.category-form :$account :$edition :$category />
                </div>
            </article>
        @endforeach
    </div>
