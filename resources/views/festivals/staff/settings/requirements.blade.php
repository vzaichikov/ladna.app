...
<x-festivals.staff.workspace :$account :$edition :permissions="$workspacePermissions">
    ...
    <div class="space-y-4">@foreach($requirements as $requirement)<article class="rounded-2xl border border-stone-200 bg-white p-5 shadow-crm"><div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><h2 class="text-lg font-semibold">{{ $requirement->name }}</h2><p class="mt-1 text-sm text-slate-500">{{ __('app.festival_input_'.$requirement->input_type->value) }} · {{ $requirement->workflowStep?->title }} · {{ $requirement->category?->name ?? __('app.all') }}</p></div><x-festivals.settings-actions :active="$requirement->is_active" :toggle-route="route('dashboard.accounts.festivals.requirements.toggle', [$account, $edition, $requirement])" :move-route="route('dashboard.accounts.festivals.requirements.move', [$account, $edition, $requirement])" /></div><details class="mt-4"><summary class="cursor-pointer text-sm font-semibold text-brand-700">{{ __('app.edit') }}</summary><div class="mt-4"><x-festivals.requirement-form :$account :$edition :$requirement /></div></details></article>@endforeach</div>
