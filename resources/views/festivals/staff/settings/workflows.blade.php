    ...
    <div class="space-y-5">
        @foreach($edition->workflows as $workflow)
            <article class="rounded-2xl border border-stone-200 bg-white p-5 shadow-crm">
                <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><h2 class="text-lg font-semibold">{{ $workflow->name }}</h2><p class="mt-1 text-sm text-slate-500">{{ trans_choice('app.festival_category_usage_count', $workflow->categories_count, ['count' => $workflow->categories_count]) }}</p></div><x-festivals.settings-actions :active="$workflow->is_active" :toggle-route="route('dashboard.accounts.festivals.workflows.toggle', [$account, $edition, $workflow])" :move-route="route('dashboard.accounts.festivals.workflows.move', [$account, $edition, $workflow])" /></div>
                <details class="mt-4"><summary class="cursor-pointer text-sm font-semibold text-brand-700">{{ __('app.edit') }}</summary><form method="POST" action="{{ route('dashboard.accounts.festivals.workflows.update', [$account, $edition, $workflow]) }}" class="mt-3 flex flex-col gap-3 sm:flex-row">@csrf @method('PUT')<input name="name" value="{{ $workflow->name }}" required class="crm-field"><input type="hidden" name="is_active" value="{{ $workflow->is_active ? 1 : 0 }}"><x-ui.button type="submit" variant="secondary">{{ __('app.save') }}</x-ui.button></form></details>
                <ol class="mt-5 space-y-3">
                    @foreach($workflow->steps as $step)
                        <li class="rounded-xl border border-stone-200 p-4">
                            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><strong>{{ $loop->iteration }}. {{ $step->title }}</strong><p class="mt-1 text-xs text-slate-500">{{ __('app.festival_step_type_'.$step->type->value) }} · {{ __('app.festival_review_mode_'.$step->review_mode->value) }} · {{ __('app.festival_review_effect_'.$step->review_effect->value) }} · {{ trans_choice('app.festival_dependency_usage_count', $step->requirement_definitions_count + $step->charge_definitions_count, ['count' => $step->requirement_definitions_count + $step->charge_definitions_count]) }}</p></div><x-festivals.settings-actions :active="$step->is_active" :toggle-route="route('dashboard.accounts.festivals.workflow-steps.toggle', [$account, $edition, $workflow, $step])" :move-route="route('dashboard.accounts.festivals.workflow-steps.move', [$account, $edition, $workflow, $step])" /></div>
                            <details class="mt-3"><summary class="cursor-pointer text-xs font-semibold text-brand-700">{{ __('app.edit') }}</summary><div class="mt-3"><x-festivals.workflow-step-form :$account :$edition :$workflow :$step /></div></details>
                        </li>
                    @endforeach
                </ol>
