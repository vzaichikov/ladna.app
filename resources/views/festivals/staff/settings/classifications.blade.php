...
<x-festivals.staff.workspace :$account :$edition :permissions="$workspacePermissions">
    ...
    <section class="rounded-2xl border border-stone-200 bg-white p-5 shadow-crm">
        ...
        <form method="POST" action="{{ route('dashboard.accounts.festivals.axes.store', [$account, $edition]) }}" class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">@csrf
            <label><span class="crm-label">{{ __('app.code') }}</span><input name="code" required class="crm-field"></label><label><span class="crm-label">{{ __('app.name') }}</span><input name="name" required class="crm-field"></label><label><span class="crm-label">{{ __('app.festival_axis_kind') }}</span><select name="kind" class="crm-field">@foreach(['style', 'age', 'level', 'entry_format', 'custom'] as $kind)<option value="{{ $kind }}">{{ __('app.festival_axis_kind_'.$kind) }}</option>@endforeach</select></label><label class="flex items-end gap-2 pb-3"><input type="checkbox" name="is_required" value="1" checked>{{ __('app.required') }}</label><div class="lg:col-span-4"><x-ui.button type="submit">{{ __('app.add') }}</x-ui.button></div>
        </form>
    </section>
