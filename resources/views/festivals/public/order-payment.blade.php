...
@section('title', __('app.festival_monopay_payment_title').' - '.$order->edition->title)
        ...
        <div class="rounded-2xl border border-stone-200 bg-white p-5 shadow-crm sm:p-7">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <h1 class="mt-2 text-2xl font-semibold sm:text-3xl">{{ __('app.festival_monopay_payment_title') }}</h1>
                    <p class="mt-2 text-sm leading-6 text-slate-500">{{ __('app.festival_monopay_payment_help') }}</p>
                </div>
            ...
            >
                <div class="flex justify-center">
                    <iframe
                        id="payFrame"
                        title="{{ __('app.festival_monopay_iframe_title') }}"
                        width="100%"
                        height="700"
                        src="{{ $pageUrl }}"
                        allow="payment *"
                        class="h-[700px] w-full max-w-[600px] rounded-3xl border-0 bg-white md:h-[600px]"
                        data-ticket-monopay-frame
                    ></iframe>
                </div>

                <div class="mt-4 flex flex-wrap justify-center gap-2">
                    <x-ui.button :href="$pageUrl" variant="secondary" target="_blank" rel="noopener noreferrer" data-ticket-monopay-direct-link>
                        <x-ui.icon name="external-link" class="h-4 w-4" />
                        {{ __('app.festival_monopay_open_direct') }}
                    </x-ui.button>
                    <x-ui.button :href="$returnUrl" variant="secondary">
                        <x-ui.icon name="arrow-left" class="h-4 w-4" />
                        {{ __('app.festival_monopay_back_to_order') }}
                    </x-ui.button>
                </div>

                <div class="mt-4 hidden rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950" data-ticket-order-poll-timeout>
                    <p data-ticket-order-poll-message aria-live="polite">{{ __('app.festival_order_confirmation_taking_longer') }}</p>
                </div>
            </div>
