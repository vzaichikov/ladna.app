    ...
    <div class="mx-auto max-w-4xl">
        ...
        <form method="POST" action="{{ $entry->exists ? route('festival.portal.entries.update', [$account->slug, $entry]) : route('festival.portal.entries.store', [$account->slug, $edition->slug]) }}" class="mt-6 space-y-6">
            ...
            <section class="rounded-2xl border border-stone-200 bg-white p-5 shadow-crm sm:p-6">
                <div class="flex items-center gap-3"><span class="flex size-8 items-center justify-center rounded-full bg-brand-600 text-sm font-bold text-white">1</span><h2 class="text-xl font-semibold">{{ __('app.festival_performance_details') }}</h2></div>
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    <label><span class="crm-label">{{ __('app.festival_category') }}</span><select name="festival_category_id" required class="crm-field"><option value="">{{ __('app.select') }}</option>@foreach($categories as $category)<option value="{{ $category->id }}" @selected((int) old('festival_category_id', $entry->festival_category_id) === $category->id)>{{ $category->name }} · {{ $category->min_members }}–{{ $category->max_members }}</option>@endforeach</select></label>
