...
@php($isGuest = $portalUser?->role === \App\Enums\FestivalPortalRole::Guest)
...
<nav class="mt-4 flex flex-wrap items-center gap-2 rounded-2xl border border-stone-200 bg-white p-3 shadow-crm" aria-label="{{ __('app.festival_portal') }}">
    ...
    @elseif($isGuest)
        <a href="{{ route('festival.portal.guest.dashboard', $account->slug) }}" class="crm-tab" @if(request()->routeIs('festival.portal.guest.dashboard')) aria-current="page" @endif>{{ __('app.festival_my_tickets') }}</a>
        <a href="{{ route('festival.portal.guest.profile.edit', $account->slug) }}" class="crm-tab" @if(request()->routeIs('festival.portal.guest.profile.*')) aria-current="page" @endif>{{ __('app.profile') }}</a>
        <a href="{{ route('public.festivals.index', $account->slug) }}" class="crm-tab">{{ __('app.festivals') }}</a>
    @else
