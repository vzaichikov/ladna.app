        ...
        <section class="mt-7 grid gap-4 md:grid-cols-2">
            @forelse ($assignments as $assignment)
                <article class="rounded-2xl border border-stone-200 bg-white p-5 shadow-crm">
                    ...
                    <div class="mt-4 flex flex-wrap gap-2"><x-ui.button :href="route('festival.portal.judging.index', [$account->slug, $assignment->edition])">{{ __('app.festival_judging') }}</x-ui.button><x-ui.button :href="route('festival.portal.battle-votes.index', [$account->slug, $assignment->edition->slug])" variant="secondary">{{ __('app.festival_battle_voting') }}</x-ui.button>@if($assignment->is_head_judge)<x-ui.button :href="route('festival.portal.judging.results.nominations.index', [$account->slug, $assignment->edition])" variant="secondary">{{ __('app.festival_nomination_assignments') }}</x-ui.button>@foreach($assignment->categories as $category)@if($category->competition_format->value === 'scored')<x-ui.button :href="route('festival.portal.judging.results.table', [$account->slug, $assignment->edition, $category])" variant="secondary">{{ __('app.festival_result_table') }} · {{ $category->name }}</x-ui.button>@endif @endforeach @endif</div>
                </article>
            @empty
