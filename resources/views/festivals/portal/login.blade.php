...
<main class="min-h-screen bg-canvas px-5 py-10">
    <div class="mx-auto max-w-lg">
        ...
        <section class="mt-8 rounded-3xl border border-stone-200 bg-white p-6 shadow-crm sm:p-8">
            <p class="text-sm font-semibold text-brand-700">{{ $account->name }}</p>
            <h1 class="mt-2 text-3xl font-semibold">{{ $isJudge ? __('app.festival_judge_login') : __('app.festival_participant_login') }}</h1>
            ...
            <div class="mt-6 border-t border-stone-100 pt-5 text-sm">
                <a href="{{ $isJudge ? route('festival.login', $account->slug) : route('festival.judge.login', $account->slug) }}" class="font-semibold text-brand-700 hover:text-brand-600">{{ $isJudge ? __('app.festival_participant_cabinet') : __('app.festival_judge_cabinet') }}</a>
                <a href="{{ route('public.festivals.index', $account->slug) }}" class="mt-3 block font-semibold text-slate-500 hover:text-slate-950">← {{ __('app.festivals') }}</a>
            </div>
        </section>
    </div>
</main>
