...
@section('content')
    ...
    <nav class="mt-6 flex gap-2 overflow-x-auto pb-1" aria-label="{{ __('app.generated_classes') }}">
        @foreach ($tabs as $tab => $label)
            <a
                href="{{ route('dashboard.accounts.scheduled-classes.index', ['account' => $account, 'tab' => $tab]) }}"
        ...
        @forelse ($scheduledClassDays as $date => $classes)
            ...
            <section>
                <div class="mb-4 flex items-center justify-between gap-3">
                    <h2 class="text-lg font-semibold text-slate-950">{{ $day->translatedFormat('l, j F') }}</h2>
                    <span class="text-sm font-semibold text-slate-500">{{ $classes->count() }}</span>
                </div>
