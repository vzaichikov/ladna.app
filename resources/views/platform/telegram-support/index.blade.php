    ...
    <x-ui.panel padding="none" id="telegram-support-alerts" class="mt-6 overflow-hidden" role="tabpanel" aria-labelledby="telegram-support-tab-alerts">
        ...
        @forelse ($alerts as $alert)
            @php
                ...
                $isFoundersAnnouncement = $alertType === \App\Enums\TelegramAlertType::FoundersAnnouncement->value;
            @endphp
            <article class="crm-row lg:grid-cols-[150px_minmax(0,1fr)_minmax(0,2fr)_minmax(0,1.4fr)_160px] lg:items-start">
                ...
                <div class="min-w-0">
                    <div class="font-semibold text-slate-950">{{ $isFoundersAnnouncement ? __('app.platform') : ($alert->account?->name ?? __('app.not_set')) }}</div>
                    <div class="mt-1 text-sm text-slate-500">{{ $isFoundersAnnouncement ? __('app.telegram_founders_destination') : ($alert->account?->slug ?? __('app.not_set')) }}</div>
                ...
                <div class="min-w-0 text-sm leading-6 text-slate-700">
                    @if ($isFoundersAnnouncement)
                        <div class="font-semibold text-slate-950">{{ __('app.telegram_alert_type_founders_announcement') }}</div>
                        <div class="text-slate-500">{{ \Illuminate\Support\Str::limit((string) data_get($alert->payload, 'source_ref'), 12, '') }}</div>
                    @elseif ($isOwnerAnnouncement)
                ...
                <div class="min-w-0 text-sm leading-6 text-slate-700">
                    <div class="font-semibold text-slate-950">
                        @if ($isFoundersAnnouncement)
                            {{ $alert->broadcastTarget?->title ?? __('app.telegram_alert_recipient_kind_founders_group') }}
                        @elseif ($isOwnerAnnouncement)
                            {{ $alert->authorization?->user?->name ?? __('app.telegram_alert_recipient_kind_studio_owner') }}
                        @else
                            {{ $alert->trainer?->name ?? __('app.trainer_not_assigned') }}
                        @endif
                    </div>
