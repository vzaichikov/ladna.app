...
@section('title', __('app.sms_payments').' - '.__('app.platform'))
...
@section('content')
    ...
    <div>
        <h1 class="crm-page-title">{{ __('app.sms_payments') }}</h1>
        <p class="crm-page-copy">{{ __('app.platform_sms_payments_copy') }}</p>
    </div>
    ...
    <form method="GET" action="{{ route('platform.sms-payments.index') }}" class="mt-6 grid gap-4 rounded-xl border border-stone-200 bg-white p-5 shadow-crm sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_auto] xl:items-end">
        <label class="block">
        ...
        <label class="block">
            <span class="crm-label">{{ __('app.sms_top_up_kind') }}</span>
            <select name="kind" class="crm-field">
                <option value="">{{ __('app.all_sms_top_up_kinds') }}</option>
                @foreach ($kinds as $kindOption)
                    <option value="{{ $kindOption->value }}" @selected($kind === $kindOption->value)>{{ __('app.sms_top_up_kind_'.$kindOption->value) }}</option>
                @endforeach
            </select>
        </label>
        ...
        @forelse ($payments as $payment)
            @php
                $paymentStatusClass = match ($payment->status->value) {
                    ...
                    'payment_failed', 'payment_cancelled', 'payment_expired', 'payment_reversed' => 'crm-status-danger',
            ...
            <article class="crm-row lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)_130px_140px_160px] lg:items-center">
                <div class="min-w-0">
                    ...
                    <div class="mt-1 break-all text-sm text-slate-500">{{ $payment->order_id }}</div>
                    @if ($payment->gateway_invoice_id)
                        <div class="mt-1 break-all text-xs font-medium text-slate-500">{{ $payment->gateway_invoice_id }}</div>
                    @endif
                </div>
