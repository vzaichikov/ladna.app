<label class="block">
    ...
    <select name="location_id" required class="crm-field">
        ...
        @foreach ($locations as $location)
            <option value="{{ $location->id }}" @selected((int) old('location_id', $serviceRoom->location_id) === $location->id)>{{ $location->name }}@unless ($location->is_active) · {{ __('app.inactive') }}@endunless</option>
        @endforeach
    </select>
...
<div class="grid gap-4 sm:grid-cols-2">
    <label class="block">
        ...
        <input name="name" value="{{ old('name', $serviceRoom->name) }}" required class="crm-field">
        @error('name') <span class="crm-help">{{ $message }}</span> @enderror
    </label>
    <label class="block">
        <span class="crm-label">{{ __('app.slug') }}</span>
        <input name="slug" value="{{ old('slug', $serviceRoom->slug) }}" class="crm-field">
...
<label class="block">
    ...
    <textarea name="description" rows="3" class="crm-field">{{ old('description', $serviceRoom->description) }}</textarea>
...
<div class="grid gap-4 sm:grid-cols-2">
    <label class="block">
        ...
        @php
            $colorValue = old('color', $serviceRoom->color);
    ...
    <label class="flex items-center gap-3 pt-8 text-sm font-medium text-slate-700">
        ...
        <input name="is_active" type="checkbox" value="1" @checked(old('is_active', $serviceRoom->is_active)) class="crm-checkbox">
        {{ __('app.active') }}
    </label>
</div>
...
<fieldset class="rounded-lg border border-stone-200 bg-slate-50 p-4">
    ...
    @if ($canManageCameraSettings)
        <p class="mt-1 text-sm leading-6 text-slate-500">{{ __('app.service_room_rtsp_camera_help') }}</p>

        <div class="mt-4 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
            <label class="block">
                <span class="crm-label">{{ __('app.rtsp_address') }}</span>
                <input name="rtsp_url" type="text" inputmode="url" value="{{ old('rtsp_url', $serviceRoom->rtsp_url) }}" placeholder="rtsp://user:password@camera-host:554/stream" class="crm-field bg-white">
            ...
            <x-ui.button
                type="submit"
                variant="secondary"
                formaction="{{ route('dashboard.accounts.service-rooms.test-camera', $account) }}"
                formmethod="POST"
            >
        ...
        <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <label class="flex items-center gap-3 text-sm font-medium text-slate-700">
                ...
                <input name="rtsp_enabled" type="checkbox" value="1" @checked(old('rtsp_enabled', $serviceRoom->rtsp_enabled)) class="crm-checkbox">
    ...
    @else
        @if (filled($serviceRoom->rtsp_url))
            <label class="mt-4 block">
                <span class="crm-label">{{ __('app.rtsp_address') }}</span>
                <input type="text" value="{{ $serviceRoom->rtsp_url }}" class="crm-field bg-white text-slate-500" readonly disabled>
            </label>
        @endif

        <p class="mt-1 text-sm leading-6 text-slate-500">
            {{ filled($serviceRoom->rtsp_url) ? __('app.rtsp_camera_change_support_notice') : __('app.rtsp_camera_add_support_notice') }}
        </p>
    @endif
</fieldset>
