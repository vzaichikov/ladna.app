...
class NormalizeCustomerClassPasses
{
    ...
    public function forPass(CustomerClassPass $customerClassPass): CustomerClassPass
    {
        return DB::transaction(function () use ($customerClassPass): CustomerClassPass {
            ...
            $shouldReactivateUsedUpPass = $customerClassPass->status === CustomerClassPassStatus::UsedUp
                && ! $isUsedUp
                && ! $isExpired
                && ! $isTotalExpired;
            $isOperationalPass = $wasActivePass || $wasFreezedPass || $shouldReactivateUsedUpPass;
            ...
            if ($wasFreezedPass && $isTotalExpired) {
                ...
                $isActive = true;
                $closedAt = null;
            } elseif ($shouldReactivateUsedUpPass) {
