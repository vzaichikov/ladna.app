...
final readonly class PlatformAnnouncementExecutionGuard
{
    ...
    public function authorize(): array
    {
        if (
            ...
            throw new InvalidArgumentException('Platform announcements are available only from a trusted CLI process.');
        }

        $origin = trim((string) getenv('LADNA_FOUNDERS_ANNOUNCEMENT_ORIGIN'));
        ...
        if ($origin !== 'platform_owner') {
            throw new InvalidArgumentException('Platform announcements require the Codex skill or a verified platform owner.');
        }

        $platformUserId = filter_var(
            getenv('LADNA_FOUNDERS_ANNOUNCEMENT_PLATFORM_USER_ID'),
            FILTER_VALIDATE_INT,
            ['options' => ['min_range' => 1]],
        );
