...
- `CustomerClassPassReservation` is the ledger between a booking and the purchased pass. Future bookings reserve sessions. Attended, no-show, and staff-cancelled bookings consume sessions. A timely authenticated customer cancellation keeps the cancelled booking in history but releases the reservation and returns the session.
