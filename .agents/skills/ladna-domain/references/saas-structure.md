...
- One `ClassPassPlan` belongs to exactly one `ScheduleKind`. Group and private lesson passes may bind to one or more explicitly selected class types of their own schedule kind; rental plans must bind to exactly one matching class type. Existing selections stay unchanged until the owner edits the plan; new class types are not automatically included.
...
- `CustomerClassPassReservation` is the ledger between a booking and the purchased pass. Future bookings reserve sessions. Attended and no-show bookings consume sessions. A timely cancellation by either studio staff or the authenticated customer keeps the cancelled booking and released reservation in history while returning the session.
