...
class ClassPassPlanTest extends TestCase
{
    ...
    public function test_class_pass_plan_catalog_queries_are_scoped_to_factory_accounts(): void
    {
        $account = Account::factory()->create();
        $otherAccount = Account::factory()->create();
        $morningSegment = ClassPassSegment::factory()->for($account)->create([
        ...
        $groupPlan = ClassPassPlan::factory()->for($account)->for($morningSegment)->create([
            'name' => 'Morning group pass',
            'schedule_kind' => 'group_class',
        ]);
        $privatePlan = ClassPassPlan::factory()->for($account)->create([
            'name' => 'Private lesson pass',
            'schedule_kind' => 'private_lesson',
        ]);
        ClassPassPlan::factory()->for($otherAccount)->create([
            'name' => 'Other studio pass',
            'schedule_kind' => 'room_rental',
        ]);

        $accountPlans = ClassPassPlan::whereBelongsTo($account);

        $this->assertSame(2, (clone $accountPlans)->count());
        $this->assertSame(1, (clone $accountPlans)->where('schedule_kind', 'group_class')->count());
        $this->assertSame(1, (clone $accountPlans)->where('schedule_kind', 'private_lesson')->count());
        $this->assertSame($morningSegment->id, $groupPlan->class_pass_segment_id);
        $this->assertTrue($privatePlan->is($accountPlans->whereKey($privatePlan->getKey())->firstOrFail()));
    }
