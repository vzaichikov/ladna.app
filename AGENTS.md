...
- Activate `ladna-payment-checkout-ui` for public Event, class-pass, or Festival checkout and ticket-delivery presentation involving responsive payment blocks, ticket iframes, provider buttons, wallet/card marks, agreements, private payment returns, QR tables, PDF sharing/printing, or payment-status UI.
...
- Run Playwright commands from the project root with `npm --prefix .codex ...` so browser binaries stay under `.codex/ms-playwright`.
- If the local browser cache is missing, run `npm --prefix .codex run pw:install`.
- Resolve app URLs with Boost `get-absolute-url` before sharing or testing a route.
- Capture desktop screenshots with `npm --prefix .codex run screenshot:desktop -- <url> .codex/output/<name>-desktop.png`.
- Capture mobile screenshots with `npm --prefix .codex run screenshot:mobile -- <url> .codex/output/<name>-mobile.png`.
- The screenshot scripts already use `--ignore-https-errors` and a 3 second wait; keep that wait for the Laravel welcome screen and other transition-heavy pages.
